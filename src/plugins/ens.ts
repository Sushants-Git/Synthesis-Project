...
export const ENSPlugin: Plugin = {
  ...
  aiDescription:
    ...
    'resolve_name: name(string) → address(string). ' +
    'resolve_batch: names(string[]) → addresses(string[]) — use for lists from sheets. ' +
    'lookup_address: address(string) → name(string). ' +
    'Wire example: ens:resolve_name→metamask:send_eth needs wire {"address":"to"}. ' +
    'ens:resolve_batch→metamask:batch_send needs wire {"addresses":"recipients"}.',
