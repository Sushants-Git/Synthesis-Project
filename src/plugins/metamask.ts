...
export const MetaMaskPlugin: Plugin = {
  ...
  aiDescription:
    ...
    'connect_wallet() → wallet_address(string) — sets sender wallet in context. ' +
    'send_eth(to*, amount*) → tx_hash — to is recipient address. ' +
    'batch_send(recipients[]*, amount?) → tx_hashes[], sent_count — recipients is array of 0x addresses. ' +
    'create_delegation(delegate*, max_amount*, caveat?) → delegation_hash. ' +
    'approve() — pauses flow for user confirmation. ' +
    'Wire: ens:resolve_name→send_eth needs wire {"address":"to"}. ' +
    'sheets/ens:resolve_batch→batch_send needs wire {"rows":"recipients"} or {"addresses":"recipients"}.',
