...
export const ChatGPTPlugin: Plugin = {
  ...
  aiDescription:
    ...
    'process(items[]*, prompt*, model?) → result, results[] — feed any array of strings into GPT with an instruction, get back structured output. ' +
    'GPT auto-detects output format from the prompt: if the prompt asks for JSON array, results[] is populated; otherwise result (string) is set. ' +
    'Use for: scoring, ranking, classifying, summarizing, extracting, transforming lists of items. ' +
    'Wire example: twitter:get_profiles→chatgpt:process needs wire {"profiles":"items"}. ' +
    'Wire example: sheets:fetch_rows→chatgpt:process needs wire {"rows":"items"}.',
